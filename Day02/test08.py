...
def getImg1(ImageUrl):
    response = requests.get(ImageUrl).content  # 获取图片
    with open(f'{name_path}.jpg', 'wb') as f:  # 保存图片,wb表示写入二进制文件
        f.write(response)
    print("下载完成")


name_path = 'img3_yellow'
if not os.path.exists(name_path):  # 判断文件夹是否存在
    os.mkdir(name_path)  # 创建文件夹
url = "http://m.sfw108.com/pic/siwameitui/58.html"
response = requests.get(url)
img_txt = BeautifulSoup(response.content, "html.parser")  # 解析网页
find = img_txt.find("ul", attrs={'class': 'tj-list cf'})  # 查找图片
# print(find)
findImg = find.find_all("img")  # 查找所有图片
# print(findImg)
...
def getUrl():
    url = "http://m.sfw108.com/pic/siwameitui/58.html"  # 图片地址
    response = requests.get(url)
    img_txt = BeautifulSoup(response.content, "html.parser")  # 解析网页
