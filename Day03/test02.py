...
def getTable(num):
    ...
    bs_find = bs.find('div', attrs={'class': 'mod-bd'})  # 找到评论区的div
    # print(bs_find)
    bs_find2 = bs_find.find_all('div', attrs={'class': 'comment-item'})  # 找到评论区的div
    # print(bs_find2)
...
# # 写入csv文件--封装
# def write_csv(movie_name):
#     path_name = "Comments"
#     if not os.path.exists(path_name):  # 判断文件夹是否存在
#         os.mkdir(path_name)  # 创建文件夹
#     with open(f'{path_name}/{movie_name}.csv', 'w', newline='', encoding='utf-8') as f:
#         pass

# ---------------------------------笔记---------------------------------
# find('div')找标签
# find_all('div')找所有标签
# find('div',attrs={'class':'comment-item'})找标签的属性
# find_all('div',attrs={'class':'comment-item'})找所有标签的属性
# find('div').find('span')找标签下的标签
# find('div').find_all('span')找标签下的所有标签
# find('div').find('span',attrs={'class':'comment-info'})找标签下的标签的属性
# find('div').find_all('span',attrs={'class':'comment-info'})找标签下的所有标签的属性
# find('div').get('title')获取标签的属性值
# find('div').text获取标签的文本内容


# ---------------------------------笔记---------------------------------
# BOM字符起到了标记文件编码的作用但是他并不属于文件的内容部分
# 将utf-8编码的文件保存为csv文件时，会在文件开头添加BOM字符，这个字符在utf-8编码中是不可见的，但是在其他编码中是可见的
# 在windows中，如果文件以BOM字符开头，那么excel会将其识别为utf-8编码，如果文件不以BOM字符开头，那么excel会将其识别为ANSI编码
# 所以我的文件写入编码改为utf-8-sig，这样就可以在excel中正常显示了
